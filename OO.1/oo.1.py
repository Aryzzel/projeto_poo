...
class Linha(Figura):
    ...
    def desenhar(self, tracejado=None):
...
class Rabisco(Figura):
    def __init__(self, pontos, corBorda):
        self.pontos = [pontos]
        self.corBorda = corBorda
...
class Circulo(Linha):           
    def desenhar(self, tracejado=None):
        x1, y1, x2, y2 = self.coordenadas
        self.raioX = abs(x1-x2)
        self.raioY = abs(y1-y2)
        raio = self.raioX + self.raioY
...
class Oval(Linha):     
    def desenhar(self, tracejado=None):
        ...
        self.raioX = abs(x1-x2)
        self.raioY = abs(y1-y2)
        x, y = self.coordenadas[0], self.coordenadas[1]
        canvas.create_oval(x-self.raioX, y-self.raioY, x+self.raioX, y+self.raioY, outline=self.corBorda, fill=self.corFill, dash=tracejado)
...
def iniciar_figura_nova(event):
    ...
    cor_p = cor_preenchimento_atual.get()
    if cor_p == "Nenhum":
        cor_p = ""

    if tipo_figura_var.get() == 'Linha':
        figura_nova = ("linha", (event.x, event.y, event.x, event.y), cor, cor_p)
    elif tipo_figura_var.get() == 'Rabisco':
        figura_nova = ("rabisco", [(event.x, event.y)], cor, cor_p)
    elif tipo_figura_var.get() == 'Círculo':
        figura_nova = ("circulo", (event.x, event.y, event.x), cor, cor_p)
    elif tipo_figura_var.get() == 'Oval':
        figura_nova = ("oval", (event.x, event.y, event.x, event.y), cor, cor_p)
    elif tipo_figura_var.get() == 'Retângulo':
        figura_nova = ("retangulo", (event.x, event.y, event.x, event.y), cor, cor_p)
...
def atualizar_figura_nova(event):
    ...
    if figura_nova[0] == "rabisco":
        ...
        figura_nova = ("linha", (figura_nova[1][0], figura_nova[1][1], event.x, event.y), cor, cor_p)
    elif figura_nova[0] == "circulo":
        figura_nova = ("circulo", (figura_nova[1][0], figura_nova[1][1],
                                   abs(figura_nova[1][0] + figura_nova[1][1] - (event.x + event.y))), cor, cor_p)
    elif figura_nova[0] == "oval":
        figura_nova = ("oval", (figura_nova[1][0], figura_nova[1][1], abs(figura_nova[1][0] - event.x),
                                abs(figura_nova[1][1] - event.y)), cor, cor_p)
    elif figura_nova[0] == "retangulo":
        figura_nova = ("retangulo", (figura_nova[1][0], figura_nova[1][1], event.x, event.y), cor, cor_p)
    desenhar_figuras()
...
def desenhar_figuras():
    ...
    for fig, values, cor, cor_p in figuras:
        if fig == "linha":
            canvas.create_line(values[0], values[1], values[2], values[3],
                               fill=cor)  # em linha, fill é a cor do traço mesmo (não tem outline)
        elif fig == "rabisco":
            canvas.create_line(values, fill=cor)
        elif fig == "circulo":
            canvas.create_oval(values[0] - values[2], values[1] - values[2], values[0] + values[2],
                               values[1] + values[2], outline=cor, fill=cor_p)  # outline = cor da borda
        elif fig == "oval":
            canvas.create_oval(values[0] - values[2], values[1] - values[3], values[0] + values[2],
                               values[1] + values[3], outline=cor, fill=cor_p)
        elif fig == "retangulo":
            canvas.create_rectangle(values[0], values[1], values[2], values[3], outline=cor, fill=cor_p)


def desenhar_figura_nova():
    fig, values, cor, cor_p = figura_nova
    if fig == "linha":
        ...
        canvas.create_oval(values[0] - values[2], values[1] - values[2], values[0] + values[2], values[1] + values[2],
                           outline=cor, fill=cor_p, dash=(4, 2))
    elif fig == "oval":
        canvas.create_oval(values[0] - values[2], values[1] - values[3], values[0] + values[2], values[1] + values[3],
                           outline=cor, fill=cor_p, dash=(4, 2))
    elif fig == "retangulo":
        canvas.create_rectangle(values[0], values[1], values[2], values[3], outline=cor, fill=cor_p, dash=(4, 2))


def incompleta(figura):
    fig, values, cor, cor_p = figura  # cor não é usada aqui, só serve pra desempacotar certo
    if fig == "linha":
...
def escolher_cor_preenchimento():
    cor_inicial = cor_preenchimento_atual.get()
    cor_inicial = "#8c8488"
    cor = colorchooser.askcolor(title="Escolha a cor de preenchimento", initialcolor=cor_inicial)
    if cor[1] is not None:
        cor_preenchimento_atual.set(cor[1])
        botao_cor_preenchimento.config(bg=cor[1], fg="black")


def remover_preenchimento():
    cor_preenchimento_atual.set("")
    botao_cor_preenchimento.config(bg="SystemButtonFace", fg="black")
...
# bordas com cor
cor_borda_atual = StringVar(root,
...
cor_preenchimento_atual = StringVar(root, value=None)
...
botao_sem_preenchimento = Button(frame, text='Transparente', command=remover_preenchimento)
botao_sem_preenchimento.grid(column=3, row=1, sticky=W, **paddings)

# Área de desenho
canvas = Canvas(frame, bg='white', width=600, height=600)
canvas.grid(column=0, row=2, columnspan=5, sticky=W, **paddings)
